    ...
    if (peer->failed) {
        if (peer->soopt) {
            if (!ucr->quiet)
                uwsgi_log("[uwsgi-%s] unable to connect() to node \"%.*s\" (%d retries): %s\n", ucr->short_name,
                          (int) peer->instance_address_len, peer->instance_address, peer->retries,
                          strerror(peer->soopt));
        }
        ...
        if (ucr->subscriptions && peer->un && peer->un->len > 0) {
            ...
            if (peer->un->death_mark == 0)
                uwsgi_log("[uwsgi-%s] %.*s => marking %.*s as failed\n", ucr->short_name, (int) peer->key_len,
                          peer->key, (int) peer->instance_address_len, peer->instance_address);
            ...
            uwsgi_log("[uwsgi-%s] %.*s => marking %.*s as failed\n", ucr->short_name, (int) peer->key_len, peer->key,
                      (int) peer->instance_address_len, peer->instance_address);
        }
...
struct uwsgi_rb_timer *corerouter_reset_timeout_fast(struct uwsgi_corerouter *ucr, struct corerouter_peer *peer,
                                                     time_t now) {
    cr_del_timeout(ucr, peer);
...
int uwsgi_cr_set_hooks(struct corerouter_peer *peer, ssize_t (*read_hook)(struct corerouter_peer *),
                       ssize_t (*write_hook)(struct corerouter_peer *)) {
    struct corerouter_session *cs = peer->session;
...
struct corerouter_session *corerouter_alloc_session(struct uwsgi_corerouter *ucr, struct uwsgi_gateway_socket *ugs,
                                                    int new_connection, struct sockaddr *cr_addr,
                                                    socklen_t cr_addr_len) {
...
void uwsgi_corerouter_loop(int id, void *data) {
    ...
    if (ucr->stats_server && i_am_the_first) {
        ...
        if (tcp_port) {
            ...
            ucr->cr_stats_server = bind_to_unix(ucr->stats_server, uwsgi.listen_queue, uwsgi.chmod_socket,
                                                uwsgi.abstract_socket);
        }

        event_queue_add_fd_read(ucr->queue, ucr->cr_stats_server);
        uwsgi_log("*** %s stats server enabled on %s fd: %d ***\n", ucr->short_name, ucr->stats_server,
                  ucr->cr_stats_server);
    }
    ...
    if (ucr->emperor_socket) {
        ...
        if (colon) {
            ...
            ucr->emperor_socket_addr_len = socket_to_in_addr(ucr->emperor_socket, colon, 0,
                                                             &ucr->emperor_socket_addr.sa_in);
        }
        else {
...
#endif
#endif
                        struct corerouter_session *cr = corerouter_alloc_session(ucr, ugs, new_connection,
                                                                                 (struct sockaddr *) &cr_addr,
                                                                                 cr_addr_len);
...
//
// 在: fasterouter中调用: fastrouter_init
//     通过结构体指针实现C的类似继承的算法
//     
        ...
        for (i = 0; i < UMAX16; i++) {
            ...
            while (s_slot) {
                ...
                while (s_node) {
                    ...
                    if (uwsgi_stats_keylong_comma(us, "last_requests", (unsigned long long) s_node->last_requests))
                        goto end0;
