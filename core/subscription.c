...
struct uwsgi_subscribe_node *uwsgi_get_subscribe_node(struct uwsgi_subscribe_slot **slot, char *key, uint16_t keylen,
                                                      struct uwsgi_subscription_client *client) {
    ...
    while (node) {
        ...
        // is the node alive ?
        if ((node->len == 0 && (subscription_age > uwsgi.subscription_tolerance_inactive)) ||
            (node->len > 0 && (subscription_age > uwsgi.subscription_tolerance))) {
            if (node->death_mark == 0) {
                if (node->len > 0) {
                    uwsgi_log(
                            "[uwsgi-subscription for pid %d] %.*s => marking %.*s as failed (no announce received in %d seconds)\n",
                            (int) uwsgi.mypid, (int) keylen, key, (int) node->len, node->name,
                            uwsgi.subscription_tolerance);
                }
                else if (node->vassal_len > 0) {
                    uwsgi_log(
                            "[uwsgi-subscription for pid %d] %.*s => marking vassal %.*s as failed (no announce received in %d seconds)\n",
                            (int) uwsgi.mypid, (int) keylen, key, (int) node->vassal_len, node->vassal,
                            uwsgi.subscription_tolerance_inactive);
                }
            }
            node->failcnt++;
            node->death_mark = 1;
...
struct uwsgi_subscribe_node *uwsgi_get_subscribe_node_by_name(struct uwsgi_subscribe_slot **slot, char *key,
                                                              uint16_t keylen, char *val, uint16_t vallen) {
...
// 添加: 订阅的Node
struct uwsgi_subscribe_node *uwsgi_add_subscribe_node(struct uwsgi_subscribe_slot **slot,
                                                      struct uwsgi_subscribe_req *usr) {

    struct uwsgi_subscribe_slot *current_slot = uwsgi_get_subscribe_slot(slot, usr->key,
                                                                         usr->keylen), *old_slot = NULL, *a_slot;
    ...
    if (current_slot) {
                ...
                if (has_address_and_vassal) {
                    if (usr->address_len == node->len && !memcmp(usr->address, node->name, node->len)) {
                        ...
                        if (usr->clear) {
                            node->len = 0;
                            uwsgi_log(
                                    "[uwsgi-subscription for pid %d] %.*s => cleared address for vassal node: %.*s (weight: %d, backup: %d)\n",
                                    (int) uwsgi.mypid, usr->keylen, usr->key, (int) usr->vassal_len, usr->vassal,
                                    usr->weight, usr->backup_level);
                        }
                    }
                    else {
                        ...
                        uwsgi_log(
                                "[uwsgi-subscription for pid %d] %.*s => updated vassal node: %.*s with address %.*s (weight: %d, backup: %d)\n",
                                (int) uwsgi.mypid, usr->keylen, usr->key, (int) usr->vassal_len, usr->vassal,
                                (int) usr->address_len, usr->address, usr->weight, usr->backup_level);
                    }
                }
        ...
        uwsgi_log("[uwsgi-subscription for pid %d] %.*s => new node: %.*s (weight: %d, backup: %d)\n",
                  (int) uwsgi.mypid, usr->keylen, usr->key, usr->address_len, usr->address, usr->weight,
                  usr->backup_level);
        if (node->notify[0]) {
            char buf[1024];
            int ret = snprintf(buf, 1024, "[subscription ack] %.*s => new node: %.*s", usr->keylen, usr->key,
                               usr->address_len, usr->address);
            if (ret > 0 && ret < 1024)
        ...
        uwsgi_log("[uwsgi-subscription for pid %d] new pool: %.*s (hash key: %d, algo: %s)\n", (int) uwsgi.mypid,
                  usr->keylen, usr->key, current_slot->hash, uwsgi_subscription_algo_name(current_slot->algo));
        if (usr->address_len > 0) {
            uwsgi_log("[uwsgi-subscription for pid %d] %.*s => new node: %.*s (weight: %d, backup: %d)\n",
                      (int) uwsgi.mypid, usr->keylen, usr->key, usr->address_len, usr->address, usr->weight,
                      usr->backup_level);
            ...
            uwsgi_log("[uwsgi-subscription for pid %d] %.*s => new vassal node: %.*s (weight: %d, backup: %d)\n",
                      (int) uwsgi.mypid, usr->keylen, usr->key, usr->vassal_len, usr->vassal, usr->weight,
                      usr->backup_level);
        }
...
static int uwsgi_subscription_ub_fix(struct uwsgi_buffer *ub, uint8_t modifier1, uint8_t modifier2, uint8_t cmd,
                                     char *sign) {
#ifdef UWSGI_SSL
        if (sign) {
...
static struct uwsgi_buffer *uwsgi_subscription_ub(char *key, size_t keysize, uint8_t modifier1, uint8_t modifier2,
                                                  uint8_t cmd, char *socket_name, char *sign, char *sni_key,
                                                  char *sni_crt, char *sni_ca) {
    ...
    if (uwsgi.subscription_notify_socket) {
        if (uwsgi_buffer_append_keyval(ub, "notify", 6, uwsgi.subscription_notify_socket,
                                       strlen(uwsgi.subscription_notify_socket)))
            goto end;
    }
...
void uwsgi_send_subscription_from_fd(int fd, char *udp_address, char *key, size_t keysize, uint8_t modifier1,
                                     uint8_t modifier2, uint8_t cmd, char *socket_name, char *sign, char *sni_key,
                                     char *sni_crt, char *sni_ca) {
    ...
    struct uwsgi_buffer *ub = uwsgi_subscription_ub(key, keysize, modifier1, modifier2, cmd, socket_name, sign, sni_key,
                                                    sni_crt, sni_ca);
...
void uwsgi_send_subscription(char *udp_address, char *key, size_t keysize, uint8_t modifier1, uint8_t modifier2,
                             uint8_t cmd, char *socket_name, char *sign, char *sni_key, char *sni_crt, char *sni_ca) {
    uwsgi_send_subscription_from_fd(-1, udp_address, key, keysize, modifier1, modifier2, cmd, socket_name, sign,
                                    sni_key, sni_crt, sni_ca);
}
...
void uwsgi_subscribe(char *subscription, uint8_t cmd) {
    ...
    // --subscribe-to 192.168.0.100:7000:@mappings.txt 
    // 这种模式暂不考虑
    if (subscription_key[1] == '@') {
                ...
                if (lines[i] == 0) {
                    if (keysize > 0) {
                        if (key[0] != '#' && key[0] != '\n') {
                            ...
                            uwsgi_send_subscription(udp_address, key, keysize, uwsgi_str_num(modifier1, modifier1_len),
                                                    0, cmd, socket_name, sign, NULL, NULL, NULL);
                            modifier1 = NULL;
                            modifier1_len = 0;
                        }
                    }
        ...
        // 最终参数：
        // subscription_key, modifier1, sign
        //
        // 如何发送订阅消息?
        uwsgi_send_subscription(udp_address,
                                subscription_key + 1, strlen(subscription_key + 1),
                                uwsgi_str_num(modifier1, modifier1_len), 0,
                                cmd, socket_name,
                                sign, NULL, NULL, NULL);

        // 恢复: subscription_key 中的数据
        if (modifier1)
            modifier1[-1] = ',';
        if (sign)
            sign[-1] = ',';
...
// iphash
static struct uwsgi_subscribe_node *uwsgi_subscription_algo_iphash(struct uwsgi_subscribe_slot *current_slot,
                                                                   struct uwsgi_subscribe_node *node,
                                                                   struct uwsgi_subscription_client *client) {
...
static struct uwsgi_subscribe_node *uwsgi_subscription_algo_lrc(struct uwsgi_subscribe_slot *current_slot,
                                                                struct uwsgi_subscribe_node *node,
                                                                struct uwsgi_subscription_client *client) {
        ...
        if (!node->death_mark) {
            if (node->backup_level == backup_level) {
                if (min_rc == 0 || node->reference < min_rc) {
                    ...
                    chosen_node = node;
                    if (min_rc == 0 && !(node->next && node->next->reference <= node->reference &&
                                         node->next->last_requests <= node->last_requests))
                        break;
                }
            }
...
static struct uwsgi_subscribe_node *uwsgi_subscription_algo_wlrc(struct uwsgi_subscribe_slot *current_slot,
                                                                 struct uwsgi_subscribe_node *node,
                                                                 struct uwsgi_subscription_client *client) {
    uint64_t backup_level = 0;
    uint64_t has_backup = 0;
        ...
        if (!node->death_mark) {
            if (node->backup_level == backup_level) {
                ...
                if (min_rc == 0 || ref < min_rc) {
                    ...
                    chosen_node = node;
                    if (min_rc == 0 &&
                        !(node->next && next_node_ref <= ref && node->next->last_requests <= node->last_requests))
                        break;
                }
            }
            else if (node->backup_level > backup_level && (!has_backup || has_backup > node->backup_level)) {
...
static struct uwsgi_subscribe_node *uwsgi_subscription_algo_wrr(struct uwsgi_subscribe_slot *current_slot,
                                                                struct uwsgi_subscribe_node *node,
                                                                struct uwsgi_subscription_client *client) {
    uint64_t backup_level = 0;
    uint64_t has_backup = 0;
...
struct uwsgi_subscribe_node *(*uwsgi_subscription_algo_get(char *name, size_t len))(struct uwsgi_subscribe_slot *,
                                                                                    struct uwsgi_subscribe_node *,
                                                                                    struct uwsgi_subscription_client *) {
    struct uwsgi_string_list *usl = NULL;
    uwsgi_foreach(usl, uwsgi.subscription_algos) {
        // 返回指定 name的 subscription algorithm
        if (!uwsgi_strncmp(usl->value, usl->len, name, len)) {
            return (struct uwsgi_subscribe_node *(*)(struct uwsgi_subscribe_slot *, struct uwsgi_subscribe_node *,
                                                     struct uwsgi_subscription_client *)) usl->custom_ptr;
        }
    }
    return NULL;
}

void uwsgi_register_subscription_algo(char *name, struct uwsgi_subscribe_node *(*func)(struct uwsgi_subscribe_slot *,
                                                                                       struct uwsgi_subscribe_node *,
                                                                                       struct uwsgi_subscription_client *)) {
