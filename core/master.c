...
void uwsgi_master_check_mercy() {
    ...
    for (i = 1; i <= uwsgi.numproc; i++) {
        // 检查: workers的状态
        // 如果: 被cursed, 并且在: no_mercy_at之后还没有死掉，则master强制杀死该进程
        //
    ...
    while (uwsgi_sock) {
        ...
        if (uwsgi_sock->queue > 0 && uwsgi_sock->queue >= uwsgi_sock->max_queue) {
            uwsgi_log_verbose("*** uWSGI listen queue of socket \"%s\" (fd: %d) full !!! (%llu/%llu) ***\n",
                            uwsgi_sock->name, uwsgi_sock->fd, (unsigned long long) uwsgi_sock->queue,
                            (unsigned long long) uwsgi_sock->max_queue);
        }

        if (uwsgi.alarm_backlog) {
            char buf[1024];
            int ret =
                            snprintf(buf, 1024, "listen queue of socket \"%s\" (fd: %d) full !!! (%llu/%llu)", uwsgi_sock->name,
                                            uwsgi_sock->fd, (unsigned long long) uwsgi_sock->queue,
                                            (unsigned long long) uwsgi_sock->max_queue);
            if (ret > 0 && ret < 1024) {
...
int master_loop(char **argv, char **environ) {
    ...
    // 首先初始化
    // 1. 设置processname, 在top中看到的就不再是很长的一个命令，而是 nginx:master/nginx:slave之类的东西了
    ...
    for (;;) {
        ...
        // cheaper management
        if (uwsgi.cheaper && !uwsgi.status.is_cheap && !uwsgi_instance_is_reloading && !uwsgi_instance_is_dying
                        && !uwsgi.workers[0].suspended) {
        ...
        if (diedpid == 0) {
            ...
            for (i = 0; i < ushared->files_monitored_cnt; i++) {
                if (!ushared->files_monitored[i].registered) {
                    ushared->files_monitored[i].fd = event_queue_add_file_monitor(uwsgi.master_queue,
                                    ushared->files_monitored[i].filename, &ushared->files_monitored[i].id);
            ...
            for (i = 0; i < ushared->timers_cnt; i++) {
                if (!ushared->timers[i].registered) {
                    ushared->timers[i].fd = event_queue_add_timer(uwsgi.master_queue, &ushared->timers[i].id,
                                    ushared->timers[i].value);
            ...
            for (i = 0; i < ushared->rb_timers_cnt; i++) {
                if (!ushared->rb_timers[i].registered) {
                    ushared->rb_timers[i].uwsgi_rb_timer = uwsgi_add_rb_timer(rb_timers,
                                    uwsgi_now() + ushared->rb_timers[i].value, &ushared->rb_timers[i]);
            ...
            if (uwsgi_master_check_workers_deadline())
                someone_killed++;
            if (uwsgi_master_check_gateways_deadline())
                someone_killed++;
            if (uwsgi_master_check_mules_deadline())
                someone_killed++;
            if (uwsgi_master_check_spoolers_deadline())
                someone_killed++;
            if (uwsgi_master_check_crons_deadline())
                someone_killed++;
            ...
            if (((uwsgi.subscriptions || uwsgi.subscriptions2)
                            && ((uwsgi.master_cycles % uwsgi.subscribe_freq) == 0 || uwsgi.master_cycles == 1))
                            && !uwsgi_instance_is_reloading && !uwsgi_instance_is_dying
                            && !uwsgi.workers[0].suspended) {
            ...
            if (uwsgi.queue_store && uwsgi.queue_filesize && uwsgi.queue_store_sync
                            && ((uwsgi.master_cycles % uwsgi.queue_store_sync) == 0)) {
                if (msync(uwsgi.queue_header, uwsgi.queue_filesize, MS_ASYNC)) {
                    uwsgi_error("msync()");
                }
            }
        ...
        if (uwsgi_instance_is_reloading || uwsgi_instance_is_dying) {
            ...
            uwsgi_log("worker %d buried after %d seconds\n", thewid,
                            (int) (uwsgi_now() - uwsgi.workers[thewid].cursed_at));
        ...
        if (WIFEXITED(waitpid_status) && WEXITSTATUS(waitpid_status) == UWSGI_FAILED_APP_CODE) {
            ...
            if (WIFSIGNALED(waitpid_status)) {
                uwsgi_log("DAMN ! worker %d (pid: %d) died, killed by signal %d :( trying respawn ...\n", thewid,
                                (int) diedpid, (int) WTERMSIG(waitpid_status));
            } else {
...
// 和 uwsgi_brutally_reload_workers 相比， 多了一个信号屏蔽，以及给子进程的信号是不一样的: SIGHUP/SIGINT
